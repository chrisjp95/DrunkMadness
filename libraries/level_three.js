function levelThree(){
  //enemy one
  if (frameCount%70 === 0){
    var enemy = createSprite(random(width), 0, 40, random(40, 100));
    enemy.setSpeed(4,90);
    enemy.life = 200;
    enemies.add(enemy);
  ...
  //enemy two
  if (frameCount%70 === 0){
    var enemy = createSprite(random(width), 0, 40, random(40, 105));
    enemy.setSpeed(4,90);
    enemy.life = 200;
    enemies.add(enemy);
    enemy.addAnimation('enemyTwo_frames',enemyTwo_animation);
    enemy.scale = .5;
  }
  //enemy three
  if (frameCount%70 === 0){
    var enemy = createSprite(random(width), 0, 40, random(40, 110));
    enemy.setSpeed(4,90);
    enemy.life = 200;
    enemies.add(enemy);
    enemy.addAnimation('enemyThree_frames',enemyThree_animation);
    enemy.scale = .5;
    }
    //powerUp
  ...
  for(var i = 0;i < enemies.length;i++){
    if(random(100) < enemyVelProb){
      enemies[i].velocity.x += random(-enemyVelChange,enemyVelChange)
    }
  }
